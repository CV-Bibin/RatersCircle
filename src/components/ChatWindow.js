...
import videoCallSound from './Sounds/videocall.wav';
import messagePopSound from './Sounds/message.wav'
import { handleMessageXP } from '../utils/xpSystem';
...
export default function ChatWindow({ activeGroup, currentUser, userData }) {
  ...
  const handleSendWithXP = async (...args) => {
    // 1. Send the message using your existing logic
    await handleSendMessage(...args);

    // 2. Trigger the XP System (Spam check + Streak)
    if (currentUser?.uid) {
      await handleMessageXP(currentUser.uid);
    }
  };
  // 👆 END OF NEW FUNCTION 👆
  ...
  // --- 🔊 UPDATED: SOUND NOTIFICATIONS (Ringtone & Message Pop) ---
  useEffect(() => {
    // 1. Safety Check
    ...
    // 3. Common Checks (Must be Recent & Not sent by me)
    const isRecent = lastMsg.createdAt && (Date.now() - lastMsg.createdAt < 10000); // 10s buffer
    const isNotMe = currentUser && lastMsg.senderId !== currentUser.uid;

    if (isRecent && isNotMe) {
        // A. Check if it is a "Video Meeting Started" message
        const isCallStart = lastMsg.text && lastMsg.text.includes("started a Video Meeting");

        if (isCallStart) {
            // PLAY RINGTONE
            const audio = new Audio(videoCallSound);
            audio.volume = 0.5;
            audio.play().catch(err => console.log("Ringtone blocked by browser:", err));
        } else {
            // B. Otherwise, it's a normal message -> PLAY POP SOUND
            const audio = new Audio(messagePopSound);
            audio.volume = 0.4; // Slightly lower volume for text
            audio.play().catch(err => console.log("Message sound blocked:", err));
        }
    }
  ...
  return (
    ...
    <div className={`flex flex-col h-full ${THEMES[currentTheme].bg} relative overflow-hidden rounded-3xl shadow-xl border border-white/50 transition-colors duration-500`}>
        ...
        <div className="flex items-center gap-2 ml-4 shrink-0">
          ...
          {isManager && (
            <button
              ...
              className={`p-2 rounded-full transition-all duration-200 ${isRestricted
      ...
      <ChatInput
        onSendMessage={handleSendWithXP}
